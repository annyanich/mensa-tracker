...
def parse_table(menu_table):
    """
    Parse the name, price, allergens, category, etc. of every menu entry
    in a table.
    ...
    for category_row in table_body.find_all('tr', recursive=False)[1:]:
        ...
        for td, date in zip(entries_monday_to_friday, date_range):
            ...
            for menu_item_div in menu_item_divs:
                item = item_from_string(menu_item_div.text)
                item['category'] = get_category_name_from_row(category_row)
                item['date_valid'] = date
                if not item['price']:
                    item['price'] = get_price_of_category(category_row)
                yield item
...
def get_date_range_from_string(date_range_string):
    ...
    date_strings = re.findall(regex, date_range_string)
...
def get_menu_entries_for_five_days(row_of_menu):
    cells = row_of_menu.find_all('td', recursive=False)
    assert len(cells) == 6, (
...
def get_price_of_category(row):
    first_cell_in_row = row.find('td', recursive=False)
...
def item_from_string(item_string):
    ...
    :return: A MenuEntry Item object used by Scrapy
    """
    ...
    item_name = remove_price_from_string(item_name)
...
def fix_garbled_german_characters(string):
    """
    This is necessary due to some kind of encoding issue that I have been
    unable to resolve.
    :param string: A string that may contain Umlauts or Eszets.
    :return: The same string, with possibly garbled characters fixed.
    """
...
def remove_price_from_string(string):
